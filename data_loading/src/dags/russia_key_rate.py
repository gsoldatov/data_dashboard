...
from data_loading.src.tasks.russia_key_rate.fetch_file import (
    fetch_file_task,
)
from data_loading.src.tasks.russia_key_rate.parse_data import (
    parse_data_task,
)
from python_common.src import get_config

DAG_ID = "russia_key_rate"
...
@dag(
    ...
    catchup=False,
    tags=["russia_key_rate"],
)
def russia_key_rate_dag() -> None:
    config = get_config()

    fetch_file_task(config) >> parse_data_task(config)


russia_key_rate = russia_key_rate_dag()
