...
from data_loading.src.tasks.russia_consumer_price_index.fetch_file import (
    fetch_file_task,
)
...
DAG_ID = "russia_consumer_price_index"
...
@dag(
    ...
    catchup=False,
    tags=["russia_consumer_price_index"],
)
def russia_consumer_price_index_dag() -> None:
    config = get_config()

    fetch_file_task(config) >> parse_data_task(config)


russia_consumer_price_index = russia_consumer_price_index_dag()
