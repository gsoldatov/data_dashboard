...
from data_loading.src.tasks.russia_labor_workforce.fetch_file import (
    fetch_file_task,
)
from data_loading.src.tasks.russia_labor_workforce.parse_data import (
    parse_data_task,
)
from python_common.src import get_config

DAG_ID = "russia_labor_workforce"
...
@dag(
    ...
    catchup=False,
    tags=["russia_labor_workforce"],
)
def russia_labor_workforce_dag() -> None:
    config = get_config()

    fetch_file_task(config) >> parse_data_task(config)


russia_labor_workforce = russia_labor_workforce_dag()
