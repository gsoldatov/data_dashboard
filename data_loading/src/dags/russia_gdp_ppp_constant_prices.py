...
from data_loading.src.tasks.russia_gdp_ppp_constant_prices.fetch_file import (
    fetch_file_task,
)
...
DAG_ID = "russia_gdp_ppp_constant_prices"
...
@dag(
    ...
    catchup=False,
    tags=["russia_gdp_ppp_constant_prices"],
)
def russia_gdp_ppp_constant_prices_dag() -> None:
    config = get_config()

    fetch_file_task(config) >> parse_data_task(config)


russia_gdp_ppp_constant_prices = russia_gdp_ppp_constant_prices_dag()
