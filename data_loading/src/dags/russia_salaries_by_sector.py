...
from data_loading.src.tasks.russia_salaries_by_sector.fetch_file import (
    fetch_file_task,
)
from data_loading.src.tasks.russia_salaries_by_sector.parse_data import (
    parse_data_task,
)
from python_common.src import get_config

DAG_ID = "russia_salaries_by_sector"
...
@dag(
    ...
    catchup=False,
    tags=["russia_salaries_by_sector"],
)
def russia_salaries_by_sector_dag() -> None:
    config = get_config()

    fetch_file_task(config) >> parse_data_task(config)


russia_salaries_by_sector = russia_salaries_by_sector_dag()
