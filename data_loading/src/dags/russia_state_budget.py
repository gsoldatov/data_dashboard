...
from data_loading.src.tasks.russia_state_budget.fetch_page import fetch_page_task
from data_loading.src.tasks.russia_state_budget.parse_page_data import (
    parse_page_data_task,
)
from python_common.src import get_config

DAG_ID = "russia_state_budget" 
...
def russia_state_budget_dag() -> None:
    config = get_config()

    fetch_page_task(config) >> parse_page_data_task(config)


russia_state_budget = russia_state_budget_dag()
...
if __name__ == "__main__":
    ...
    # Does not work since v3.1.0
    # russia_state_budget.test()