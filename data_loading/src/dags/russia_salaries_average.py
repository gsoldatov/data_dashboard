...
from data_loading.src.tasks.russia_salaries_average.fetch_file import (
    fetch_file_task,
)
...
DAG_ID = "russia_salaries_average"
...
@dag(
    ...
    catchup=False,
    tags=["russia_salaries_average"],
)
def russia_salaries_average_dag() -> None:
    config = get_config()

    fetch_file_task(config) >> parse_data_task(config)


russia_salaries_average = russia_salaries_average_dag()
