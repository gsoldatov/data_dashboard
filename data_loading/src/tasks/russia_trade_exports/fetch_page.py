...
def fetch_page_task(config: Config | None = None) -> None:
    """
    Fetches an HTML page with Russia's export trade data by partner country
    ...
    url = (
        ...
        f"{current_year}"
        "/TradeFlow/Export/Partner/BY-COUNTRY/Indicator/XPRT-TRD-VL"
    )

    logger.info("Fetching Russia trade exports page from %s", url)

    data_dir = config.visualization_data_directory
    save_path = data_dir / "russia_trade_exports" / "page.html"
