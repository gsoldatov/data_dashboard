...
def fetch_file_task(config: Config | None = None) -> None:
    """
    Fetches an Excel file with Russia's GDP data (constant prices).
    """
    logger = logging.getLogger("airflow.task")
    config = config or get_config()

    # NOTE: update URL in MDX pages, when changing this link
    url = "https://rosstat.gov.ru/storage/mediabank/VVP_god_s1995-2025.xlsx"

    logger.info("Fetching Russia GDP xlsx from %s", url)

    data_dir = config.visualization_data_directory
    save_path = data_dir / "russia_gdp_constant_prices_rub" / "gdp.xlsx"
