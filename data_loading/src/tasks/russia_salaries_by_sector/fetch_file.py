...
def fetch_file_task(config: Config | None = None) -> None:
    """
    Fetches an Excel file with Russia's salary-by-sector data.
    """
    ...
    logger.info("Fetching Russia salaries by sector xlsx from %s", url)

    data_dir = config.visualization_data_directory
    save_path = data_dir / "russia_salaries_by_sector" / "salaries.xlsx"
