...
def fetch_file_task(config: Config | None = None) -> None:
    """
    Fetches a zip archive with World Bank GDP (PPP) data and extracts
    the CSV file for indicator NY.GDP.MKTP.PP.KD.
    """
    ...
    url = (
        ...
        "/NY.GDP.MKTP.PP.KD?downloadformat=csv"
    )

    logger.info("Fetching World Bank GDP PPP zip from %s", url)

    data_dir = config.visualization_data_directory
    save_path = data_dir / "russia_gdp_ppp_constant_prices" / "gdp.csv"
...
def _extract(
    ...
    """
    Extracts the CSV file starting with 'API_NY.GDP.MKTP.PP'
    from the zip archive and saves it to dest_path.
    """
    prefix = "API_NY.GDP.MKTP.PP"
