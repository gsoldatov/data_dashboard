...
def parse_data_task(config: Config | None = None) -> None:
    """
    Parses the World Bank GDP (PPP) CSV into JSON.
    """
    logger = logging.getLogger("airflow.task")

    config = config or get_config()
    data_dir = config.visualization_data_directory
    csv_path = data_dir / "russia_gdp_ppp_constant_prices" / "gdp.csv"
    json_path = data_dir / "russia_gdp_ppp_constant_prices" / "gdp.json"

    logger.info("Parsing World Bank GDP PPP CSV file")

    try:
        if not csv_path.is_file():
            raise RuntimeError("World Bank GDP PPP CSV file does not exist")
        ...
        logger.info("Saved GDP PPP data to %s", json_path)
    except Exception:
...
def _parse(df: pandas.DataFrame) -> list[dict[str, Any]]:
    """
    Parses a DataFrame with World Bank GDP (PPP) data.
