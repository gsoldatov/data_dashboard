...
def test_product_code_to_name() -> None:
    ...
    assert product_code_to_name("44-49_Wood") == "Wood"
    assert product_code_to_name("27-27_Fuels") == "Fuels"
    assert product_code_to_name("16-24_FoodProd") == "Food Products"
    assert product_code_to_name("39-40_PlastiRub") == "Plastic or Rubber"
    assert product_code_to_name("84-85_MachElec") == "Machines and Electronics"
    assert product_code_to_name("86-89_Transport") == "Transportation"
    # Fallback for unknown code
    assert product_code_to_name("99-99_Unknown") == "Unknown"


def test_wood_empty_years_skipped(mock_wood_html: str) -> None:
    """Test that years with empty values are excluded from output."""
    entries = parse_product_page(mock_wood_html, "Wood")
...
def test_fuels_starts_later(mock_fuels_html: str) -> None:
    ...
    entries = parse_product_page(mock_fuels_html, "Fuels")
    years = {e["year"] for e in entries}
...
def test_combined_products(mock_wood_html: str, mock_fuels_html: str) -> None:
    ...
    wood_entries = parse_product_page(mock_wood_html, "Wood")
    fuels_entries = parse_product_page(mock_fuels_html, "Fuels")
