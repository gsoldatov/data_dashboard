"""
Test cases for World Bank USD GDP parsing logic
"""
import sys
...
def mock_df() -> pandas.DataFrame:
    """Fixture that provides the mock World Bank GDP CSV as a DataFrame."""
    mock_file_path = (
        PROJECT_ROOT
        / "data_loading/tests/mocks/mock_data"
        / "russia_gdp_constant_prices_usd.csv"
    )
...
def test_parse_filters_by_rus_country_code(mock_df: pandas.DataFrame) -> None:
    ...
    # RUS has values for 2000, 2001, 2003 → 3 entries
    assert len(result) == 3
...
def test_extracts_only_non_empty_values(mock_df: pandas.DataFrame) -> None:
    ...
    assert 2002 not in years  # empty in mock data
    assert 2004 not in years  # empty in mock data
